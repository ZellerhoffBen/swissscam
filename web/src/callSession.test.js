...
test("only heard segments reach the model, in order, with accumulated text", async (t) => {
  ...
  const results = [];
  const stop = startCall({
    audio,
    ...callbacks(),
    onDetection: (result) => results.push(result),
  });
  t.after(stop);
  await flush();
...
test("upload parses split UTF-8 events and ignores future cumulative text", async (t) => {
  ...
  const event = {
    type: "segment",
    text: "Future text must not be displayed",
    segment: { ...segments[0], text: "Grüezi" },
  };
  ...
  const received = [];
  await transcribeRecording(file, new AbortController().signal, (s) =>
    received.push(s),
  );
...
test("failed or interrupted transcription is not reported as complete", async (t) => {
  ...
  t.mock.method(
    ...
    "fetch",
    async () =>
      new Response('data: {"type":"error","error":"Invalid audio"}\n\n'),
  );
  await assert.rejects(
    transcribeRecording(file, new AbortController().signal, assert.fail),
    /Invalid audio/,
  );
...
test("an analysis error stops playback and reports a visible error", async (t) => {
  t.mock.method(globalThis, "fetch", async (url) =>
    url === "/api/transcribe"
      ? json({ segments })
      : new Response('{"detail":"Classifier unavailable"}', { status: 503 }),
  );
  const audio = new Recording();
  let error = "";
  const stop = startCall({
